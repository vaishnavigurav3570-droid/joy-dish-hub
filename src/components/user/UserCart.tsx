...
export function UserCart({
  ...
  handlePlaceOrder, placing, handleRequestMore, isRestaurantOpen
}: any) {
  return (
    <>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} className="fixed bottom-0 left-0 right-0 z-50 bg-card rounded-t-3xl p-6 max-h-[85vh] overflow-y-auto shadow-2xl border-t border-border/50" style={{ paddingBottom: 'calc(1.5rem + env(safe-area-inset-bottom, 0px))' }}>
              ...
              {cart.length === 0 ? (
                ...
                <div className="space-y-4">
                  ...
                  {!isOrderConfirmed ? (
                    <Button
                      className={`w-full text-primary-foreground rounded-2xl h-14 text-base font-bold shadow-lg shadow-primary/20 mt-2 ${!isRestaurantOpen ? 'bg-muted text-muted-foreground shadow-none' : 'gradient-warm'}`}
                      onClick={handlePlaceOrder}
                      disabled={placing || !!activeOrderId || !isRestaurantOpen} // Simplified condition
                    >
                      {!isRestaurantOpen ? <span className="flex items-center gap-2">🚫 Shop is Closed</span> :
                      ...
                      ) : <><Send className="h-4 w-4 mr-2" /> {activeOrderId ? 'Waiting...' : orderType === 'preorder' ? '🔥 Place Pre-Order' : 'Place Order'}</>}
                    </Button>
                  ) : (
                    <Button
                      className={`w-full rounded-2xl h-14 text-base font-bold mt-2 ${!isRestaurantOpen ? 'bg-muted text-muted-foreground shadow-none' : 'gradient-cool text-accent-foreground'}`}
                      onClick={handleRequestMore}
                      disabled={!isRestaurantOpen}
                    >
                      {!isRestaurantOpen ? <span className="flex items-center gap-2">🚫 Shop is Closed</span> : <><PackagePlus className="h-4 w-4 mr-2" /> Request More Items</>}
                    </Button>
                  )}
                </div>
              )}
